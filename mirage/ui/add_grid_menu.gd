...
var grid_width = 0
var grid_height = 0
signal bag_update(_item_,_item_array_size)
...
var type_bag  : RigidBody
func _init_func(type_bag_ : RigidBody):
	type_bag = type_bag_
...
func insert_item(item):
	if items.find(item) != -1:
		...
		if is_grid_space_available(g_pos.x, g_pos.y, item_size.x, item_size.y):
					...
					items.sort()
					start_item_size = start_item_size + Item.get_item(item.name_).value
					if type_bag:
						if type_bag.discount.find(item.name_) != -1:
								start_item_size = item_array_size +  Item.get_item(item.name_).value * .5
						PlayeerStat.emit_signal("player_grabed_tem_from_item_ground",null)

					get_parent().drop_item_in_children(Item.get_item(item.name_))
...
func grab_item(pos):
	...
	if item == null:
		return
	if !AntithesiSystem.is_player_able_to_trade:
			get_parent().get_node("disbaled").play()
			return
	...
	if 	type_bag != null:
		if start_item_size - Item.get_item(item.name_).value  <= 0 :
						insert_item_at_first_available_spot_at_after_ready(item)
						get_parent().get_node("item_missing").play()
						print("not enough")
						return null
		...
		return item
			...
			PlayeerStat.emit_signal("player_grabed_tem_from_item_ground",Item.get_item(item.name_))
