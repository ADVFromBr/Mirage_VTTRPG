...
signal current_collision(enable,pos)
...
func build_mesh():
	get_tree().current_scene.init_universe(self)
...
func build(var data: PatchData):
	...
	mesh_array_pool_first = vertices[0] 
	mesh = ArrayMesh.new()
...
func get_mesh():
		return  mesh_array_pool_first

var mesh_array_pool_first
