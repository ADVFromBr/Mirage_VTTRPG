...
var _min_distance_to_generate_collider: float 

   # Distance to viewer at which this node subdivides.
...
func _init(var parent: QuadNode, var direction: Vector3, \
	...
	_min_distance_to_generate_collider = Const.MIN_GENERATE_COLLISION
...
func visit():
	...
	var  distance_= _viewer_node.global_transform.origin.distance_to(_terrain_manager.global_transform.origin + _center)
	

	var viewer_generate_range: bool = distance < Const.MIN_GENERATE_COLLISION
	...
	if !Engine.editor_hint:
				if !viewer_generate_range:
					if terrain.get_children().size() > 0:
							terrain.get_tree().get_current_scene().get_node("Unverse").reset_path()
							terrain.emit_signal("current_collision",true,_terrain_manager.global_transform.origin + _center)
							for i in terrain.get_children():
								i.queue_free()

				elif viewer_generate_range:
					if terrain.get_children().size() <= 0:
							terrain.build_mesh()
							terrain.get_tree().get_current_scene().get_node("Unverse")._init_a_star(1011,2.5)
							terrain.get_tree().get_current_scene().init_universe(terrain)
							terrain.emit_signal("current_collision",true,_terrain_manager.global_transform.origin + _center)
